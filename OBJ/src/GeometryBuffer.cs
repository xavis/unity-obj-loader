	...
	public void PopulateMeshes(GameObject[] gs, Dictionary<string, Material> mats) {
		...
		for(int i = 0; i < gs.Length; i++) {
			...
			/*if (od.groups.Count == 1) {
				...
				if (gd.materialName == null || gd.materialName == "(null)"){
					//gs [i].GetComponent<Renderer>().material = mats["default"];
					Debug.Log("none");
				} else {
				...
				if (gd.faces.Count % 3 == 0) {
					int gcount = globalCount;
					for (int j = gcount; j < triangles.Length+gcount; j++) {
						triangles [j] = j;
						globalCount++;
					}
				} else {
					triangles = new int[Convert.ToInt32(gd.faces.Count*1.5)];
					int count = 1;
					int[] trs = new int[2];
					int length = 0;
					int gcount = globalCount;
					for (int j = gcount; j < gd.faces.Count + gcount; j++, count++, length++) {
						Debug.Log (gd.faces.Count + " - " + triangles.Length + " - " + globalCount + " - " + length);
						if (count < 4) {
							triangles [length] = j;
							if (count == 1) {
								trs [0] = j;
							...
							count = 0;
							triangles[length] = trs[0];
							triangles[length + 1] = j;
							triangles[length + 2] = trs [1];
							length = length + 2;
				...
				if(od.groups[j].materialName != null && mats.ContainsKey(od.groups[j].materialName))
					sml[j] = mats[od.groups[j].materialName];

					if (!od.quad) { //TRIANGLE FACES
						...
						int length = Convert.ToInt32((od.groups[j].faces.Count*1.5));
